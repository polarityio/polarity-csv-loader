...
async function cleanupOldFiles(directory, simulate) {
  return new Promise((resolve, reject) => {
    let command = `ls -t | sed -e '1,10d' | xargs -rd '\n' rm`;
    if (simulate) {
      command = `ls -t | sed -e '1,10d' | xargs -rI file echo 'Cleaning up file'`;
    }
...
async function moveFile(completedDir, filePath, simulate = false) {
  ...
  const newFile = path.join(completedDir, `${fileBaseName}-${Date.now()}.${fileExt}`);
